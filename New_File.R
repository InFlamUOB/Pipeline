...
mortality<-c(16,5,43,54,136,181,148,154,132,384,316,292,311)
PostB<-match(mortality,Hazeldine_1h$ID) 
ones<-rep("N",77)
ones[PostB]<-"Y"
savee<-as.factor(ones)
...
Final_LASSO1<-filter(Final_LASSO,n>35)

outputVenn2<-venn(list(EN= Final_EN$All, LASSO = Final_LASSO$All))

Final_EN<-mEN[order(-mEN$n),]
Final_EN1<-filter(Final_EN,n>35)
Final_Plot_Names<-filter(Final_EN,n>25)
...
corr <-cor(Betas_select2[,1:2])
corr2 <-cor(Betas_select2[,1:2],use = "complete.obs")
P_Cor<-cor_pmat(Betas_select2[,1:2]) #para que: significance: ggcorrplot(corr,p.mat=P_Cor,tl.cex=12)

ggcorrplot(corr,title="Median",tl.cex=6,tl.srt = 90,p.mat = P_Cor, sig.level = 0.01, insig = "blank")
ggcorrplot(corr2,title="NAs",tl.cex=6,tl.srt = 90,p.mat = P_Cor, sig.level = 0.01, insig = "blank")
